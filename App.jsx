...
export default function App() {
  ...
  const renderInteractiveText = (inputText) => {
    if (!result || !result.glossary || result.glossary.length === 0) return inputText;
    
    let elements = [inputText];
    
    result.glossary.forEach(gItem => {
      const escapedTerm = gItem.term.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&');
              ...
              <div className="results-container">
                ...
                {activeTab === 'summary' && (
                  <div>
                    ...
                    <div className="summary-text">
                      <p>{renderInteractiveText(result.simplifiedSummary)}</p>
                    </div>
                    ...
                    <div style={{ marginTop: '2.5rem' }}>
                      ...
                      {result.glossary && result.glossary.length > 0 ? (
                        <div className="glossary-list">
                          {result.glossary.map((gItem, idx) => (
                            <div key={idx} className="glossary-item">
                              <div className="glossary-term">{gItem.term}</div>
                              <div className="glossary-def">{gItem.definition}</div>
                              {gItem.analogy && <div className="glossary-analogy">💡 {gItem.analogy}</div>}
                            </div>
                          ))}
                        </div>
                      ) : (
                ...
                {activeTab === 'values' && (
                  <div>
                    ...
                    {groupLabValues(result.labValues).map((group) => (
